...
from upload.forms import NewUploadForm, UploadDetailForm
...
def get_upload_details(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and request.session.get('upload_file', False):
            ...
            file = temp_file.file
            file.open('r')
            for line in temp_file.file.readlines():
                new_page = Page()
                data = json.loads(line)
                new_page.url = data['url']
                new_page.body = data['body']
                new_page.head = data['head']
